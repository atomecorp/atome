...
module Sanitizer
  ...
  def validation(property, values)
    # allow = false
    # if property == :drm
    ...
    #
    #   # new_atome = set(value[0] => value[1])
    #   # puts   new_atome
    # end
    # puts 'each value we  create an atome to be validated and added inside the current atome (property)'
    # puts "values is : #{values.class}"

    values.each do |value|
      # FIXME: catch the ID instead of texting if it's a method
      # FIXME: do not use send but create Atome manually to optimise speed
      if Atome.method_defined?(value[0])
        send(value[0], value[1])
      else
    ...
    # values[values.keys[0]].each do |value|
    #   puts "value: #{value}"
    #   send(value[0], value[1])
    # end
    # p values[0]
    atomisation(property, values)
  end
  ...
  def self.atome_stack(atome_name, params, parent = :view)
    params = add_essential_properties(atome_name, params, parent)
    # TO_DO: reorder particle now
    # TO_DO: recursive test to check there no atome within the params p
    # params = { "#{atome_name}s" => Atome.new()}
    puts params[:parent]
    Atome.new({ "#{atome_name}s" => { atome_name => Atome.new } })
    # puts 'we send here to create and attach atome here !!!'
    # params
    # Atome.new(params)
  end

  def particle_stack(particle_name, value)
    "particle_stack : #{particle_name}:  #{value}"
