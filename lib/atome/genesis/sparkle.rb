...
def atome_infos
  ...
  puts "IP adress: #{Atome::ip_address}"
...
#   A.sync({ action: :authentication, data: { email: 'jeezs@atome.one' } }) do |email|
#     puts "email received : #{email}"
#   end
#   A.sync({ action: :authorization, data: { password: password } }) do |pass|
