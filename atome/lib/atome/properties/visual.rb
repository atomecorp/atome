...
module Nucleon
  module Core
    module Photon
      ...
      def render params = nil, refresh = true
        if params || params == false
          if params == false || params == :false
            ...
            unless a_found.include? atome_id[:value]
              enliven(true)
            end
          end
          @render = params
      ...
      #       @color = params
      #     end
      #     broadcast(atome_id[:value] => {color: params, private: false})
      #     if refresh
      ...
      #   if params || params == false
      #     @opacity = params
      #     broadcast(atome_id[:value] => {opacity: params, private: false})
      #     if refresh
      ...
      def x params = nil, refresh = true
        if params || params == false
          broadcast(atome_id[:value] => {x: params, private: false})
      ...
      def y params = nil, refresh = true
        if params || params == false
          ...
          # now we assign the value
          broadcast(atome_id[:value] => {y: params, private: false})
      ...
      def xx params = nil, refresh = true
        if params || params == false
          ...
          ## now we assign the value
          broadcast(atome_id[:value] => {xx: params, private: false})
      ...
      def yy params = nil, refresh = true
        if params || params == false
          ...
          ## now we assign the value
          broadcast(atome_id[:value] => {yy: params, private: false})
      ...
      def z params = nil, refresh = true
        if params || params == false
          @z = params
          broadcast(atome_id[:value] => {z: params, private: false})
          if refresh
      ...
      def width params = nil, refresh = true
        if params || params == false
          @width = params
          broadcast(atome_id[:value] => {width: params, private: false})
          if refresh
      ...
      def height params = nil, refresh = true
        if params || params == false
          @height = params
          broadcast(atome_id[:value] => {height: params, private: false})
          if refresh
      ...
      def size params = nil, refresh = true
        if params || params == false
          @size = params
          broadcast(atome_id[:value] => {size: params, private: false})
          if refresh
      ...
      def position params = nil, refresh = true
        if params || params == false
          @position = params
          broadcast(atome_id[:value] => {position: params, private: false})
          if refresh
      ...
      def overflow params = nil, refresh = true
        if params || params == false
          @overflow = params
          broadcast(atome_id[:value] => {overflow: params, private: false})
          if refresh
      ...
      def fill params = nil, refresh = true
        if params || params == false
          ...
          broadcast(atome_id[:value] => {fill: params, private: false})
          if refresh
...
# line below (self.y) is a quick patch to patch a bug
          ...
          broadcast(atome_id[:value] => {fit: params, private: false})
          return self
        else
          @fit
        end
      ...
      #  if params || params == false
      #    @rotate = params
      #    broadcast(atome_id[:value] => {rotate: params, private: false})
      #    if refresh
      ...
      def rotate_treatment(params)
        ...
        broadcast(atome_id[:value] => { rotate: params })
      end
      ...
      def shadow params = nil, refresh = true
        if params || params == false
          ...
          broadcast(atome_id[:value] => {shadow: params, private: false})
          if refresh
      ...
      def border params = nil, refresh = true
        if params || params == false
          @border = params
          broadcast(atome_id[:value] => {border: params, private: false})
          if refresh
      ...
      def smooth params = nil, refresh = true
        if params || params == false
          @smooth = params
          broadcast(atome_id[:value] => {smooth: params, private: false})
          if refresh
      ...
      def blur params = nil, refresh = true
        if params || params == false
          @border = params
          broadcast(atome_id[:value] => {blur: params, private: false})
          if refresh
      ...
      def level params = nil, refresh = true
        if params || params == false
          @level = params
          broadcast(atome_id[:value] => {level: params, private: false})
          if refresh
