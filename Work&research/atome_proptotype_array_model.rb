class Atome
  ...
  def initialize
    ...
    # @atome << { a_id:  identity  }
    # @atome  << { type: :shape}
    # @atome << { identity_generator + "p" => { type: :preset, data: :box } }
  ...
  def color val = nil
    if val.nil?
      prop_found = []
      @atome.each do |property|
        if property[:type] == :color
          prop_found << property[:data]
        end
        # if value[:type] == :color
        #   if val == :a_id
        #     return value[:data]
      ...
      puts " - we must sanitize the val:  #{val}"
      @atome << { type: :color, data: val, id: identity, x: 0, y: 0 }
    end
  end
