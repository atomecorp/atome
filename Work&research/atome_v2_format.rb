...
#   end
#
#   def parent(value = nil, add = nil)
#     getter_setter(:parent, value) do
#       atome_sanitizer(:parent, value) do |sanitized_value|
#         sanitized_value = pre_processor(:parent, sanitized_value)
#         puts sanitized_value
#         # if the pre_processor decide that the value shouldn't be store and render it return nil
#         if sanitized_value
#           check_add :parent, add
#           atome_creation(:parent, sanitized_value)
#         end
#         if property_render
...
# # this module ....
# module Electron
#   def pre_processor(property, value)
#     property
#     value
#   end
#
...
# end
#
# # a = Molecule.new({ type: :shape, preset: :box, child: %i[sphere circle], color: %i[red yellow], left: 30, top: 66 })
# # a.color({ add: :green })
# # a.color(:pink)
# # a=Molecule.new({})
#
# # b = Molecule.new({ type: :shape, preset: :circle, color: :black, left: 15, top: 99 })
# # c = Molecule.new({ type: :text,  color: { red: 0, green: 1, x: 33 }, left: 15, top: 99 })
# # d = Molecule.new({ type: :text,  color: [{ red: 0, green: 1, x: 33 }, :green], left: 15, top: 99 })
#
# # puts a.inspect
# # puts b.preset
# # a.left([88, 44, 22, 11])
...
# # # puts
# # puts a.check
# # # b = Molecule.new({ type: :shape, preset: :circle, color: :orange, left: 90, y: 9 })
# # a = Molecule.new({ type: :shape, preset: :box, color: %i[red yellow], left: [30,55], top: 66 })
