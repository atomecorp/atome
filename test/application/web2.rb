    ...
    articles_list&.each do |article_id|
      unless Universe.atomes.key?(article_id)
        ...
        layout_params = {
          ...
          cells: {
            particles: { margin: margin_found, overflow: :hidden, color: default_values[:back_color], smooth: 0, }
          }
          # cells: {
        ...
        article_content.each_with_index do |(item_id, content), index|
          ...
          when :content
            ...
            if content_found_copy[:component] && content_found_copy[:component][:padding]
              padding_found = content_found_copy[:component][:padding]
